...
import { useParams, useRouter } from "next/navigation" // Correct hooks for Next.js 13+ app dir
import { Button } from "@/components/ui/button"
import { Progress } from "@/components/ui/progress"
import { getTimesheetDetails, TimesheetDetails } from "@/lib/data"
import { ChevronLeft, Plus, MoreHorizontal } from "lucide-react"
import { Badge } from "@/components/ui/badge"
import { cn } from "@/lib/utils"
// import { DailyTaskList } from "@/components/timesheet/daily-task-list" // Will create next
...
export default function TimesheetDetailPage() {
    ...
    // Group tasks by day for easier rendering
    // We already have tasks with dates in data.tasks
    const daysMap = new Map<string, typeof data.tasks>()

    // Initialize map with all days in range to ensure empty days show up
    for (let i = 0; i < 5; i++) {
                ...
                <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
                    ...
                    <div className="space-y-6">
                        {Array.from(daysMap.entries()).map(([dateStr, tasks]) => {
                            ...
                            const isToday = new Date().toDateString() === dateStr // Just for fun, though mock data is 2024
                            ...
                            return (
                                <div key={dateStr} className="flex flex-col md:flex-row gap-4 border-b border-gray-50 pb-6 last:border-0">
                                    ...
                                    <div className="flex-grow space-y-3">
                                        {tasks.map(task => (
                                            <div key={task.id} className="group flex items-center justify-between bg-white border border-gray-100 rounded-lg p-3 hover:border-blue-200 transition-colors shadow-sm">
                                                ...
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    {/* Dropdown Menu Trigger - Placeholder for now */}
                                        ...
                                        {/* Add Task Button */}
                                        <button className="w-full py-3 my-2 border border-dashed border-gray-200 rounded-lg text-sm font-medium text-gray-400 hover:text-blue-600 hover:border-blue-200 hover:bg-blue-50/50 transition-all flex items-center justify-center gap-2 group">
